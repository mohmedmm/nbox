...
[![简体中文](https://img.shields.io/badge/Language-简体中文-red.svg)](#)
...
**nbox** 是一款专为视频画面打造的轻量级神经画质增强与实时超分辨率工具。不同于传统生硬锐化算法带来的白边光晕（Halo），也不同于过度生成的失真模型，**nbox** 专注于**微质感重塑**（毛发细丝、皮肤纹理、远景文本、织物细节），在自然细腻的基调下还原画面清晰度。
...
- **可交互绿色小方块水印 `[ ■ nbox: ON ]`**：直接点击屏幕角落图标即可随时切换增强开关；亦可在扩展弹窗中一键隐藏水印。
- **🎬 专业播放器无缝串联**：自动探测本机安装的 **KMPlayer、MPC-HC、MPC-BE、PotPlayer、VLC、MPV**，超分完成即可自动唤起播放。
- **黑曜石专业工作室界面 (Obsidian Studio)**：高格调深色 Glassmorphism 设计，内置实时动态分屏对比滑块与帧率监控仪表盘。
...
- Python 3.10 或更高版本
- 系统已安装并配置 [FFmpeg](https://ffmpeg.org/download.html) 环境变量
...
4. 打开 Bilibili、YouTube 或任意视频网站，画面将自动激活神经超清渲染与可点击的绿色方块 `[ ■ nbox: ON ]`！

### 4. Tampermonkey 油猴脚本（兼容 Firefox / Safari / Brave）
